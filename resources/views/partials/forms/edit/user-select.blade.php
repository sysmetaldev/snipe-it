<div id="assigned_user" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}" {!! (isset($style))
    ? ' style="' .e($style).'"' : '' !!}>

    {{ Form::label($fieldname, $translated_name, array('class' => 'col-md-3 control-label')) }}

    @can('change', \App\Models\User::class)
    <div class="col-md-6{{((isset($required)) && ($required=='true')) ? ' required' : '' }}">
        @else
    <div class="col-md-6{{  ((isset($required)) && ($required=='true')) ? ' required' : '' }}" readonly>
    @endcan
            <select class="js-data-ajax" data-endpoint="users" data-placeholder="{{ trans('general.select_user') }}"
                name="{{ $fieldname }}" style="width: 100%" id="assigned_user_select" aria-label="{{ $fieldname }}">
        ...
        {!! $errors->first($fieldname, '<div class="col-md-8 col-md-offset-3"><span class="alert-msg"
